...
class VendorSettingsController extends Controller
{
    ...
    */
    public function index()
    {
        ...
        return view('vendor.settings.account.account-edit')->with([
            'user' => $user
        ]);
    ...
    public function update(Request $request , $id){
        ...
        $response = Account::put($id , $body , true );

        if($response->status ==200 ){
            // session()->put('user')
            AuthApi::setuser($response->user);
            Session::flash('response', array( "status"  => 200, "action" => "success" ,"message" => 'Account Updated Successfully'));
